...
class Learner():
    ...
    def populate_params(self, params):
        ...
        self.oob_dir = path_params['oob_dir']
        ...
        self.oob_template = os.path.join(self.oob_dir, path_params['oob_template'])
        ...
        self.ms_atlas_filename = path_params['ms_atlas_filename']
    ...
    def load_globals(self):
        ...
        #load the atlas mask
        if len(self.ms_atlas_filename) == 0:
            self.ms_atlas = self.atlas_mask
        else:
            self.ms_atlas, _ = nrrd.read(self.ms_atlas_filename)
        ...
        self.ms_atlas    = temp_data3D.transform(self.ms_atlas, scaling = "A").squeeze(0)
        ...
        self.ms_atlas    = self.ms_atlas.to(self.device)
    ...
    def loop(self, batch, grad = True, save = -1, return_outputs = False, test = False):
        if grad:
            ...
            loss = self.loss_mngr.total_loss(images, trans_pred, def_field, train = grad, record = (not test))
            loss.backward()            
            self.optimizer.step()
        else:
            with torch.no_grad():
    ...
    def export(self, return_loss = False, include_valid = False):
        if return_loss:
            temp = []
    ...
    def evaluate(self):
        ...
        oob_names = [self.oob_template.replace("%s", keyword) for keyword in self.test_keywords if len(self.oob_dir) > 0]
